...
(function() {
    ...
    async function loadAll(onTotal, onArticle) {
        ...
        return { articles: arts, idMap: buildIdMap(arts), listError: false };
    }
    ...
    function ensureLoaded() {
        ...
        if (cached) {
            ...
            // 若存在直达目标且缓存命中 → 立即进入阅读视图（内容来自缓存）
            if (pendingPostId && idMap[pendingPostId]) {
                hideProgress();
                openArticle(idMap[pendingPostId]);
            } else if (!readingArticle) {
                // 列表态或占位态：先渲染列表（若不在占位中）
                if (document.getElementById('blogReadView').style.display !== 'block') {
                    renderList();
                }
            }
        }
        // 2) 静默网络刷新（进度条显示检查中；若已有缓存则快速刷新，无缓存时也是首载进度）
        setProgress('indeterminate', cached ? '正在检查博客更新…' : '正在获取文章列表…');
        loadAll(
            function(total) {
                totalCount = total;
            ...
            function(a, loadedCount) {
                ...
                // 无缓存首载时增量渲染；有缓存（已展示快照）时不重复 push
                if (!cached) {
                    articles.push(a);
                    renderLoading();
                }
            ...
            loaded = true;
            if (result.listError) {
                // 网络失败：若已有缓存则保留并提示；否则报错
                hideProgress();
                if (cached) {
                    if (!readingArticle && !pendingPostId) renderList();
                    return;
                }
                const container = document.getElementById('blogCardList');
                if (container) {
            ...
            writeCache(articles);
            hideProgress();
            // 若当前正在阅读（缓存或占位打开的目标）→ 用最新数据重渲染/填充
            if (pendingPostId) {
                ...
                if (target) {
                    if (readingArticle) {
                        // 已在阅读（缓存命中打开）：内容有更新则重渲染
                        if (target.body !== readingArticle.body || target.title !== readingArticle.title) openArticle(target);
                    } else {
                        openArticle(target); // 占位态：填充最新内容
                    }
                } else {
